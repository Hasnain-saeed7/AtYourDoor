      ...
      <nav className="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link href="/browse" className="bg-green-600 hover:bg-green-700 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-green-100">
              + Book a Worker
            </Link>
            <SignOutButton />
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'In Progress', value: stats.inProgress, color: 'bg-purple-50 text-purple-700', icon: '🔧' },
            { label: 'Completed', value: stats.completed, color: 'bg-green-50 text-green-700', icon: '✅' },
          ].map((stat) => (
            <div key={stat.label} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <span className="text-2xl">{stat.icon}</span>
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${stat.color}`}>
                  {stat.value}
                </span>
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-50 flex items-center justify-between">
            ...
            <Link href="/browse" className="text-green-600 hover:text-green-700 text-sm font-semibold">
              + New Booking
            </Link>
          </div>
          ...
          {bookings.length === 0 ? (
            <div className="text-center py-20">
              ...
              <Link href="/browse" className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-3 rounded-xl transition-all">
                Browse Workers
              </Link>
            </div>
          ) : (
